...
- `codex-environment.mjs`: read `<cwd>` from `agent.session.header.cwd` (the cwd stamped on the Session header at session creation, see `packages/core/session/src/types.ts:104`) instead of `process.cwd()`. Previously the `<cwd>` field in `<environment_context>` reflected the Node process's cwd rather than the workspace the session was created in, which could drift from the user's intent under chdir / subagent fork / remote exec. This brings the cwd source into line with upstream codex's session-scoped `<cwd>` semantics (`codex-rs/core/src/context/world_state/environment.rs`). `process.cwd()` is retained as the fallback for the theoretical case where the Session header has no cwd recorded.
